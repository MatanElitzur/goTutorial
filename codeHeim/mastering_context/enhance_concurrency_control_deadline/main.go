...
// Execute go run ./codeHeim/mastering_context/enhance_concurrency_control_deadline/main.go
// This example demonstrates how to use context to cancel a slow search operation after a timeout
func main() {
	// Create a context with a deadline
	deadline := time.Now().Add(3 * time.Second)
	ctx, cancelFunc := context.WithDeadline(context.Background(), deadline)
	defer cancelFunc() //cancel the context when the main function is done, ctx will expire at the specified deadline, regardless of when WithDeadline was called.
